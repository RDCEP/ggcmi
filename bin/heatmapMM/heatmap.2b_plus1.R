heatmap.2b_plus1 <- function (x, Rowv = TRUE, Colv = if (symm) "Rowv" else TRUE, 
                              ...
                              lmat = NULL, lhei = NULL, lwid = NULL, extrafun = NULL, ...) 
{
  ...
  # adding labels for axis 2 as extra arguement
  
  
  if(!is.null(second.label)){
    axis(2,iy,labels = second.label,las=2,line=-0.5+offsetRow,tick=0,
         cex.axis=cexRow,hadj=1,padj=adjRow[2])
  }
  if (is.null(srtRow)) {
  ...
  # add white bar between first column (for "best"), the new model column, and the rest
  if (!missing(colsep)) 
    for (csep in colsep) rect(xleft = csep + 0.5, ybottom = 0, 
                              xright = csep + 0.5 + sepwidth[1] + if(csep==1 | csep==3) 1 else 0, ytop = ncol(x) + 
                                1, lty = 1, lwd = 1, col = if(csep==1|csep==3) "white" else sepcolor, border = sepcolor)
  ...
  if (key) {
    ...
    if (density.info == "density") {
      ...
      if (is.null(key.title)) 
        key.title <- "Color Key and Density Plot"
      if (!is.na(key.title)) 
        title(key.title)
      par(cex = 0.5)
      if (is.null(key.ylab)) 
      ...
      # do not consider best column and the 2 fake columns for separation in histogram
      h <- hist(x[-c(1:3),], plot = FALSE, breaks = breaks)
