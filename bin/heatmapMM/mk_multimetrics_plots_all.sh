  ...
  for v in tscorr varratio rmse; do
    for m in none VS; do
      for c in mai ric soy whe mil sor; do
        echo $g, $v, $m, $c
        ./multimetrics_plot.py -d /project/joshuaelliott/ggcmi/processed/multimetrics -v $v -x model -y climate,agcfsr,agmerra,cfsr,erai,grasp,princeton,watch,wfdei.cru,wfdei.gpcc -c gadm0_index,$g -o scen -c corr_methods,$m -c time_range,full -c crop,$c -o detr_methods
        mv *.png /project/joshuaelliott/ggcmi/processed/multimetrics_plots_all
      done
    done
  done
done
