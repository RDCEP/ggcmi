...
outdir=/project/ggcmi/AgMIP.output/processed/masks/weight
...
for ((i = 0; i < ${#refnames[@]}; i++)); do
   wn=${wtsnames[$i]}
   rn=${refnames[$i]}
   ./create.masks.py -i $raydir/${rn}_weight_ray_1961-2008.nc4 -w $wtsdir/$wn.nc4 -v area_${rn} -y 1961 -o $outdir/${wn}.ray.nc4
done
...
for ((i = 0; i < ${#refnames[@]}; i++)); do
   wn=${wtsnames[$i]}
   rn=${refnames[$i]}
   ./create.masks.py -i $iizumidir/iizumi.2013JAN29.${rn}.1982-2006.30min.nc4 -w $wtsdir/$wn.nc4 -v area -y 1982 -o $outdir/${wn}.iizumi.nc4
done
