  ...
  for w in AgMERRA AgCFSR WFDEI.GPCC WFDEI.CRU Princeton GRASP WATCH ERAI CFSR; do
    for ((i = 0; i < ${#clong[@]}; i++)); do
      for v in yield; do
        ...
        bcfile=$(ls $root/AgMIP.output/processed/biascorr/gadm0/faostat/fixed/${m}*_${w}*${cshort[$i]}* 2> /dev/null)
        ...
        if [ $bcfile ]; then # bias-corrected file exists
          for ((j = 0; j < ${#irfiles[@]}; j++)); do
            mkdir -p $root/AgMIP.output/processed/rescaled/gadm0/faostat/fixed
            outfile=$root/AgMIP.output/processed/rescaled/gadm0/faostat/fixed/$(basename ${irfiles[$j]//_firr})
