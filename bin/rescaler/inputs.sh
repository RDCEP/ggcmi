  ...
  for w in AgMERRA AgCFSR WFDEI.GPCC WFDEI.CRU Princeton GRASP WATCH ERAI CFSR; do
    for ((i = 0; i < ${#clong[@]}; i++)); do
      for v in yield; do
        ...
        bcfile=$(ls $root/processed/biascorr/gadm0/faostat/${m,,}*_${w,,}*${cshort[$i]}* 2> /dev/null)
        wtfile=$root/processed/masks/weight/${clong[$i]}.nc4
        if [ $bcfile ]; then # bias-corrected file exists
          for ((j = 0; j < ${#irfiles[@]}; j++)); do
            outfile=$root/processed/rescaled/gadm0/faostat/$(basename ${irfiles[$j]//_firr})
